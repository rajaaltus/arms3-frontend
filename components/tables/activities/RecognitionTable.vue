<template>
  <div>
    <v-data-table :headers="headers" :items="recognitionsData" sort-by="updated_at" sort-desc class="elevation-1" :loading="loading" loading-text="Loading... Please wait">
      <template v-slot:[`item.updated_at`]="{ item }">
        {{ $moment(item.updated_at).fromNow() }}
      </template>
      <template v-slot:top>
        <v-toolbar flat color="#ebebeb" class="d-flex justify mt-4 pt-1" style="border-radius: 0;">
          <v-toolbar-title><span class="frm-title">Recognition - Awards and Honours</span></v-toolbar-title>
          ...
          <v-dialog v-model="dialog" fullscreen hide-overlay transition="dialog-bottom-transition">
            <v-card>
              <v-toolbar dark color="#41704e">
                ...
                <v-toolbar-title>Recognition - Awards and Honours</v-toolbar-title>
                <v-spacer></v-spacer>
              </v-toolbar>
              <v-card-text>
                <v-container>
...
export default {
  ...
  computed: {
    ...
    facultiesData() {
      return this.recognitionsData.filter((recognition) => recognition.user.userType === "FACULTY");
    },
  },
  watch: {
    dialog(val) {
  ...
  methods: {
    handleclick(item) {
      var index = this.recognitionsData.indexOf(item);
      if (item.approval_status === "Rejected") {
        Swal.fire({
          title: "Reason for Rejection",
          text: this.recognitionsData[index].rejected_reason,
        });
      }
    },
    ...
    deleteItem(item) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          this.loading = true;
          this.$store.dispatch("recognition/updateRecognition", payload).then((resp) => {
            this.loading = false;
            Swal.fire({
              title: "Success",
    ...
    async save(recognition) {
      var payload = recognition;
      // console.log(payload);
      await this.$store.dispatch("recognition/updateRecognition", payload);
