<template>
  <div>
    <v-data-table :headers="headers" :items="publicData" sort-by="updated_at" sort-desc class="elevation-1" :loading="loading" loading-text="Loading... Please wait">
      <template v-slot:[`item.updated_at`]="{ item }">
        {{ $moment(item.updated_at).fromNow() }}
      </template>
      <template v-slot:top>
        <v-toolbar flat color="#ebebeb" class="d-flex justify mt-4 pt-1" style="border-radius: 0;">
          <v-toolbar-title><span class="frm-title">Public Engagement & Outreach Activities</span></v-toolbar-title>
          ...
          <v-dialog v-model="dialog" fullscreen hide-overlay transition="dialog-bottom-transition">
            <v-card>
              <v-toolbar dark color="#41704e">
                ...
                <v-toolbar-title>Public Engagement & Outreach Activities</v-toolbar-title>
                <v-spacer></v-spacer>
              </v-toolbar>
              <v-card-text>
                <v-container>
                  <FormsPublicForm :dataFrom="staffs" :publicEngagementData="editedItem" :key="renderKey" @close="close" @save="save" />
                </v-container>
              </v-card-text>
            </v-card>
          </v-dialog>
...
export default {
  ...
  computed: {
    ...mapState({
      publicData: (state) => state.public.publicData.result,
  ...
  methods: {
    ...
    async save(publicEngagement) {
      var payload = publicEngagement;
      console.log(payload);
      await this.$store.dispatch("public/updatePublic", payload);
