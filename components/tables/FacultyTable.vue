<template>
  <div>
    <v-data-table style="border-radius: 0;" :headers="headers" :items="facultyData" sort-by="updated_at" sort-desc class="elevation-1" :loading="loading" loading-text="Loading... Please wait">
      ...
      <template v-slot:top>
        <v-toolbar flat color="#ebebeb" class="d-flex justify mt-4 pt-1" style="border-radius: 0;">
          <v-toolbar-title><span class="frm-title">Faculty and Staff served NIMHANS</span></v-toolbar-title>
          <v-divider class="mx-4" inset vertical></v-divider>
          <v-select v-model="annualYear" :items="reportYears" item-text="val" item-value="id" label="Select Year" class="justify-end mt-6" @change="reloadData()"></v-select>
          <v-divider class="mx-4" inset vertical></v-divider>
          ...
          <v-dialog v-model="dialog" fullscreen hide-overlay transition="dialog-bottom-transition">
            <v-card flat>
              <v-toolbar dark color="#41704e">
                ...
                <v-toolbar-title>Faculty and Staff served NIMHANS</v-toolbar-title>
                <v-spacer></v-spacer>
              </v-toolbar>
              <v-card-text>
                <!-- <v-container>
                  <v-row>
                    <v-col cols="3">
                      <v-text-field v-model="editedItem.faculty_name" label="Name of the official"></v-text-field>
                    </v-col>
                    <v-col cols="3">
                      <v-select v-model="editedItem.faculty_status" :items="facultyStatus" label="Status"></v-select>
                    </v-col>
                    <v-col cols="3">
                    ...
                    <v-col cols="3">
                      <v-menu ref="menu1" v-model="duration_to" :close-on-content-click="false" transition="scale-transition" offset-y min-width="290px">
                        <template v-slot:activator="{ on }">
                          <v-text-field color="green" v-model="editedItem.leaving_date" :rules="[(v) => !!v || 'Item is required']" :return-value.sync="date1" readonly label="To Date" v-on="on"></v-text-field>
                        </template>
                  ...
                  <v-hover>
                    <template v-slot:default="{ hover }">
                      <v-img :src="`${$axios.defaults.baseURL}${image_url}`" lazy-src="/image_placeholder.png" aspect-ratio="1" class="grey lighten-2" max-width="100%" max-height="400">
                ...
                <FormsFacultyForm :facultyData="editedItem" :key="renderKey" @close="close" @save="save" />
              </v-card-text>
            </v-card>
          </v-dialog>
        </v-toolbar>
...
export default {
  ...
  methods: {
    ...
    async save(faculty) {
      var payload = faculty;
      await this.$store.dispatch("faculty/updateFaculty", payload);
      if (this.$store.state.faculty.facultyData.success) {
        this.close();
        this.reloadData();
      }
    },
