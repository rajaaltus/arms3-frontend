<template>
  <v-container fluid>
    ...
    <v-row class="my-0">
      ...
      <v-col cols="12" lg="3">
        <v-select filled color="green" v-model="annualYear" :items="reportYears" item-text="val" item-value="id" label="Reporting Year" required @input="reload"></v-select>
      </v-col>
    </v-row>
      ...
      <v-col cols="12">
        <v-form ref="about" lazy-validation @submit.prevent>
          ...
          <div>
            ...
            <span v-if="departmentAbout.introduction != ''">Words: {{ departmentAbout.introduction && departmentAbout.introduction.split(" ").length }}</span>
          </div>
          <div>
            <p class="mt-3">
            ...
            <div>
              ...
              <span v-if="departmentAbout.facilities != ''">Words: {{ departmentAbout.facilities && departmentAbout.facilities.split(" ").length }}</span>
            </div>
          </div>
          <v-row>
            <v-spacer></v-spacer>
    ...
    <v-row class="center my-2 mx-1" justify="space-between" v-if="departmentAbout.id">
      <v-hover>
        <template v-slot:default="{ hover }">
          <v-img :src="`${$axios.defaults.baseURL}${img_url1}`" lazy-src="/image_placeholder.png" class="grey lighten-2" max-width="30%" max-height="300">
            <v-fade-transition>
              <v-overlay v-if="hover" absolute color="#036358">
                <v-btn @click="$refs.image1.click()">
                  {{ img_url1 ? "Edit Image" : "Upload Image" }}
                </v-btn>
                <v-btn v-if="departmentAbout.image_1" class="mt-0" x-small fab dark color="red darken-3" @click="deleteImage({ imageNo: 1, id: departmentAbout.image_1.id })">
          ...
          <v-img :src="`${$axios.defaults.baseURL}${img_url2}`" lazy-src="/image_placeholder.png" aspect-ratio="1" class="grey lighten-2" max-width="30%" max-height="300">
            <v-fade-transition>
              <v-overlay v-if="hover" absolute color="#036358">
                ...
                <v-btn v-if="departmentAbout.image_2" class="mt-0" x-small fab dark color="red darken-3" @click="deleteImage({ imageNo: 2, id: departmentAbout.image_2.id })">
                  <v-icon>mdi-delete</v-icon>
                </v-btn>
              </v-overlay>
            </v-fade-transition>
          ...
          <v-img :src="`${$axios.defaults.baseURL}${img_url3}`" lazy-src="/image_placeholder.png" class="grey lighten-2" max-width="30%" max-height="300">
            <v-fade-transition>
              <v-overlay v-if="hover" absolute color="#036358">
                ...
                <v-btn v-if="departmentAbout.image_3" class="mt-0" x-small fab dark color="red darken-3" @click="deleteImage({ imageNo: 3, id: departmentAbout.image_3.id })">
                  <v-icon>mdi-delete</v-icon>
                </v-btn>
              </v-overlay>
            </v-fade-transition>
...
export default {
  ...
  async fetch({ store }) {
    ...
    await store.dispatch("about/setAboutData", { qs: queryString });
  },
  mounted() {
  ...
  methods: {
    async deleteImage({ imageNo, id }) {
      await this.$store.dispatch("deleteFile", { id: id });
      if (imageNo == 1) {
        this.departmentAbout.image_1 = null;
        this.img_url1 = null;
      } else if (imageNo == 2) {
        this.departmentAbout.image_2 = null;
        this.img_url2 = null;
      } else {
        this.departmentAbout.image_3 = null;
        this.img_url3 = null;
      }
    },
    async setYear() {
    ...
    async addProfile() {
      if (this.newAbout && this.newAbout.id) {
        ...
        this.departmentAbout.annual_year = this.annualYear;
        this.departmentAbout.department = this.$store.state.auth.user.department;
        if (this.departmentAbout.introduction != "" && this.departmentAbout.facilities != "") {
          var payload = this.departmentAbout;
          // console.log(payload);
          await this.$store
    ...
    async reload() {
      ...
      if (this.newAbout) {
        this.departmentAbout = Object.assign({}, this.newAbout);
        if (this.$store.state.about.newAbout.image_1 !== null) this.img_url1 = this.$store.state.about.newAbout.image_1.url;
        else this.img_url1 = null;
