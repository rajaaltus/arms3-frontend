<template>
  <div>
    <v-data-table :headers="headers" :items="trainingsData" sort-by="updated_at" sort-desc class="elevation-1" :loading="loading" loading-text="Loading... Please wait">
      <template v-slot:[`item.updated_at`]="{ item }">
        {{ $moment(item.updated_at).fromNow() }}
      </template>
      <template v-slot:top>
        <v-toolbar flat color="#ebebeb" class="d-flex justify mt-4 pt-1" style="border-radius: 0;">
          <v-toolbar-title><span class="frm-title">Trainings Underwent</span></v-toolbar-title>
          ...
          <v-dialog v-model="dialog" fullscreen hide-overlay transition="dialog-bottom-transition">
            <v-card>
              <v-toolbar dark color="#41704e">
                ...
                <v-toolbar-title>Trainings Underwent</v-toolbar-title>
                <v-spacer></v-spacer>
              </v-toolbar>
              <v-card-text>
                <v-container>
                  <FormsTrainingForm :dataFrom="staffs" :trainingData="editedItem" :key="renderKey" @close="close" @save="save" />
                </v-container>
              </v-card-text>
            </v-card>
          </v-dialog>
...
export default {
  ...
  computed: {
    ...mapState({
      trainingsData: (state) => state.training.trainingsData.result,
  ...
  methods: {
    handleclick(item) {
      var index = this.trainingsData.indexOf(item);
      if (item.approval_status === "Rejected") {
        Swal.fire({
          title: "Reason for Rejection",
          text: this.trainingsData[index].rejected_reason,
        });
      }
    },
    ...
    deleteItem(item) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          this.loading = true;
          this.$store.dispatch("training/updateTraining", payload).then((resp) => {
            this.loading = false;
            Swal.fire({
              title: "Success",
